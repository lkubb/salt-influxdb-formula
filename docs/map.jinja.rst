...
We will see a quick setup to configure the ``influxdb`` formula for different DNS domain names and several roles.
...
Create per DNS configuration for ``influxdb`` formula
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Now, we can provides the per DNS domain name configuration files for the ``influxdb`` formulas under ``/srv/salt/influxdb/parameters/``.
...
.. code-block:: console
    ...
    mkdir -p /srv/salt/influxdb/parameters/dns:domain/
    ln -s dns:domain /srv/salt/influxdb/parameters/domain

We create a configuration for the DNS domain ``example.net`` in ``/srv/salt/influxdb/parameters/dns:domain/example.net.yaml``:
...
We create another configuration for the DNS domain ``example.com`` in the Jinja YAML template ``/srv/salt/influxdb/parameters/dns:domain/example.com.yaml.jinja``:
...
Create per role configuration for ``influxdb`` formula
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Now, we can provides the per role configuration files for the ``influxdb`` formulas under ``/srv/salt/influxdb/parameters/``.

We create the directory for roles:

.. code-block:: console

    mkdir -p /srv/salt/influxdb/parameters/roles
...
We create a configuration for the role ``influxdb/server`` in ``/srv/salt/influxdb/parameters/roles/influxdb/server.yaml``:
...
We create another configuration for the role ``influxdb/client`` in ``/srv/salt/influxdb/parameters/roles/influxdb/client.yaml``:
...
.. code-block:: console
    ...
    salt 'server-*' grains.append roles influxdb/server

For the clients:

.. code-block:: console

    salt 'client-*' grains.append roles influxdb/client
...
.. code-block:: console
    ...
    mv /srv/salt/influxdb/parameters/dns:domain/  '/srv/salt/influxdb/parameters/dns!domain/'
...
.. code-block:: sls
    ...
    test-does-nothing-but-display-influxdb-as-json:
      test.nop:
        - name: {{ influxdb | json }}
...
Here is an example based on `template-formula/influxdb/config/file.sls`_:
    ...
    influxdb-config-file-file-managed:
      file.managed:
        ...
        - source: {{ files_switch(['example.tmpl'],
                                  lookup='influxdb-config-file-file-managed'
                     )
                  }}
        - mode: 644
        - user: root
        - group: {{ influxdb.rootgroup }}
        - makedirs: True
        - template: jinja
        - require:
...
This ``sls`` file expose a ``influxdb`` context variable to the jinja template which could be used like this:
...
.. code-block:: jinja
    ...
    winner of the merge: {{ influxdb['winner'] }}
...
.. _template-formula/influxdb/config/file.sls: https://github.com/saltstack-formulas/template-formula/blob/master/influxdb/config/file.sls
